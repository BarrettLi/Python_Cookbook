...
# an abstract syntax tree representing the input. The grammer is small, so you'd prefer to just
...
class ExpressionEvaluator:
    ...
    a single grammer rule. Use the ._accept() method to test and accept
    the current lookahead token. Use the ._expect() method to exactly
    match and dicard the next token on on the input (or raise a SyntacError
    if it doesn't match).
    '''
